...
class CompromiseRSSTQuery implements ICompromiseRSSTQuery
{
    ...
    public function update(Request $request, int $id)
    {
        if ($id) {
            try {
                $Compromise = CompromiseRSST::findOrFail($id);
                if (auth()->check()) {
                    $rules = [
                        $this->item        => 'required|string',
                        $this->rule        => 'required|string',
    ...
    public function destroy(int $id)
    {
        if ($id) {
            try {
                ...
                $path = "storage/images/commerce/{$report->commerce_id}/report/{$report->id}/compromisesrsst/{$Compromise->id}";
        ...
        if ($id) {
            try {
                ...
                return response()->json([
                    ...
                    'data' => CompromiseRSST::query()->reportid($id)->get(),
